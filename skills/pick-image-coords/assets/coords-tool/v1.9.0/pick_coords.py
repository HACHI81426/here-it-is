...
class PickCoords:
    ...
    def __init__(self, image_path: str):
        ...
        self.root = tk.Tk()
        self.root.title(f"选点/拉框 - {Path(image_path).name} ({self.w}x{self.h})")
    ...
    def set_mode(self, mode: str) -> None:
        ...
        if mode == self.MODE_POINT:
            self.info.config(text="模式：取点 | 左键取点 | 右键撤销 | 滚轮缩放 | 中键平移 | r=矩形 | p=取点 | q/ESC 退出")
        else:
    ...
    def on_release(self, event) -> None:
        ox, oy = self.to_orig(event.x, event.y)
        self.update_magnifier(ox, oy)
        if self.mode != self.MODE_RECT or self.drag_start is None:
            return
    ...
    def build_output(self) -> tuple:
        ...
        for s in self.shapes:
            ...
            if s["desc"]:
                descs[key] = s["desc"]
        return self.origin, points, rects, descs

    def run(self) -> None:
        self.root.mainloop()
        origin, points, rects, descs = self.build_output()
        print("坐标零点:", origin)
        print("最终坐标:", points)
        print("最终矩形:", rects)
        if descs:
