...
class PickCoords:
    ...
    def build_clipboard_text(self) -> str:
        """把每个区域的信息列成一行：【名称 （坐标）（备注）】。"""
        lines = [f"坐标零点: {self.origin}"]
        p = r = c = 0
        for s in self.shapes:
            if s["kind"] == "point":
                p += 1
                name = f"点{p}"
                x, y = self.convert(*s["coords"])
                coords = f"（{x},{y}）"
            elif s["kind"] == "rect":
                r += 1
                name = f"矩形{r}"
                x1, y1, x2, y2 = self.convert_rect(s["coords"])
    ...
    def copy_all_to_clipboard(self) -> None:
        text = self.build_clipboard_text()
